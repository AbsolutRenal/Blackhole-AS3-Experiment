package test {
	 ...
	 */
	public class GridTest4 extends Sprite {
		private const NB_COLUMNS:uint = 20;
		private const NB_ROWS:uint = 15;
		private const HEXAGON_SIZE:uint = 30;
		private const CLICKED_COLOR:uint = 0x00AA00;
		private const AREA_COLOR:uint = 0xCC0000;
		private const SAFE_AREA_SIZE:uint = 2;

		private var container:Sprite;
		private var hexagonsDict:Dictionary = new Dictionary();
		private var lastClick:HexagonTest3;
		private var lastClickArea:Vector.<HexagonTest3> = new Vector.<HexagonTest3>();

		public function GridTest4() {
		...
		private function createGrid():void{
			var hex:HexagonTest3;
			var i:uint = 0;
			var j:uint = 0;
			for(i; i < NB_ROWS; i++){
				for(j; j < NB_COLUMNS; j++){
					hex = new HexagonTest3(HEXAGON_SIZE, new Point(j, i));
					hex.x = j * (HEXAGON_SIZE / 2 + (HEXAGON_SIZE / 2) * Math.cos(degToRad(60)));
					hex.y = i * (HEXAGON_SIZE * Math.sin(degToRad(60)));
					
					if((j % 2) == 1){
						hex.y += (HEXAGON_SIZE / 2) * Math.sin(degToRad(60));
					}
		...
		private function rollOutHexagon(event:MouseEvent):void {
			var target:HexagonTest3 = event.target as HexagonTest3;
		...
		private function clickHexagon(event:MouseEvent):void {
			...
			lastClick = event.target as HexagonTest3;
		...
		private function setArea(hex:HexagonTest3):void {
			var p:Point = hex.position;
			var tmpHex:HexagonTest3;
